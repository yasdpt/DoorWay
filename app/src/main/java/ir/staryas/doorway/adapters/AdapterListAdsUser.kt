...
class AdapterListAdsUser(val context: Context) : RecyclerView.Adapter<AdapterListAdsUser.MyViewHolder>() {

    var adListUser : MutableList<Ads> = mutableListOf()
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        if (adListUser[position].isPublished == "1"){
            holder.tvAdTitle.text = adListUser[position].adName + " | منتشر شده"
        } else {
            holder.tvAdTitle.text = adListUser[position].adName + " | منتشر نشده"
        }
        val formatter = DecimalFormat("#,###")
        val prcstr = adListUser[position].adPrice!!.replace(",","").toDouble()
        val formattedPrice:String = formatter.format(prcstr) + " تومان"
        holder.tvPrice.text = formattedPrice
        val images = adListUser[position].adImages!!.split(",")
        val pdate = PersianDate()
        val createdAt = adListUser[position].createdAt
        val tools = Tools()
    ...
    fun setAdListUserItems(adListUser: MutableList<Ads>){
        this.adListUser = adListUser
